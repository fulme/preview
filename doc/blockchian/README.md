...
节点通常采用TCP协议、使用8333端口与种子节点（长期稳定运行的已知节点）建立连接。
...
## 区块链
区块链的结构类似于链表，区别是链表有头指针和尾指针，而区块链只有头指针，指向前一个区块头的hash。
...
### 交易池
每个矿工通常都维护这个两个交易池，一个用于当前区块打包。
另外一个用于存放没有父交易的交易。还有可能有另外一个用于暂存当前区块容纳不小的交易。
...
- SPV节点验证
SPV节点会在该交易信息和它所在区块之间用merkle路径（见“7.7 Merkle 树”）建立一条链接。
然后SPV节点一直等待，直到序号从300,001到300,006的六个区块堆叠在该交易所在的区块之上，
并通过确立交易的深度是在第300,006区块~第300,001区块之下来验证交易的有效性。
